...
2. 授業名・授業種別（春学期 / 秋学期 / Q1〜Q4）・曜日・時限を入力
3. **保存** をクリック
...
**日程プレビュー** タブでは、各授業回の右側に出席コード入力欄があります。

- **手動入力**：入力欄をクリックして4桁の数字を直接入力
- **一括生成**：**一括生成** ボタンをクリックすると、未設定の全授業回にランダムなコードを自動生成
- **個別再生成**：各授業回の右端にある更新ボタンをクリック
...
システムが新しい出席コードを発行した場合など、設定ファイルを変更せずに一時的に別のコードを表示できます。
...
3. 移動元の授業回（元の日時）を選択
4. 移動先の日付と時限（またはカスタム開始時刻）を選択して確定
...
|---------|------|
| 出席コードフォント | フォント名とサイズ |
| 出席コードの色 | 数字の色（カラーブロックをクリックして選択） |
| 背景色 | ウィンドウの背景色 |
| 枠線の色 | 角丸枠線の色 |
| 授業名フォント | 授業名の表示フォントとサイズ |
| ウィンドウサイズ | 全体の拡大率（30%〜300%）。**＋** / **−** で10%ずつ調整 |
