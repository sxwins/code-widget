...
## [1.0.16] — 2026-03-04 JST · commit `1ed9f68`（修订自 ec3c70b）
...
- **问题**：§5.6 对三种调整类型（停课/补课/调课）的描述给人三者均等使用的印象，但实际配置文件（如 `teacher_A_config.json`）中所有调整记录均为 `reschedule` 类型，UI 也仅提供调课的添加入口。
